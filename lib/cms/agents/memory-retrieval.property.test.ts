 ...
 * window `MEMORY_LAST_MESSAGES` with `scope: "resource"`.
 *
 * This is a **[deps]** / **[container-only]** task: the live Mastra/Postgres
 * memory store cannot run here, so it is replaced by a FAKE in-memory store
 * that mirrors Mastra's documented retrieval contract exactly. The fake is
 * driven by the REAL key builder (`buildMemoryKey`) and the REAL bound constant
 * (`MEMORY_LAST_MESSAGES`) exported from `./memory`, so the property pins the
 * Retrieval_Policy our configuration actually requests rather than a restated
 * copy of it:
 *
 *   - **Isolation (Req 4.3):** retrieval returns ONLY records whose storage key
 ...
 *     records.
 *   - **Ordering (Req 4.3):** records are returned most-recent-write first
...
// Exactly one of resourceId / threadId is set, so the tagged string is a
// faithful, collision-free identity for an entity's storage key.
...
// A small id pool forces key collisions across kinds (e.g. user:1 vs lead:1,
// which must stay isolated) and repeats within a kind (so a single entity
// accumulates many records, exercising the recency bound).
