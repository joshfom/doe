...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const { category } = await fetchPostsByCategory("en", slug, 1);
  ...
  return generateBlogMetadata({
    metaTitle: `${category.name} — Blog`,
    metaDescription: `Posts in the ${category.name} category`,
...
export default async function EnCategoryArchivePage({ params, searchParams }: Props) {
  const { slug } = await params;
  const { page: pageParam } = await searchParams;
  const page = Math.max(1, parseInt(pageParam || "1", 10) || 1);
  const pageSize = 12;

  const { posts, total, category } = await fetchPostsByCategory("en", slug, page, pageSize);
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-12">
      <div className="mb-10">
        <p className="text-xs font-medium uppercase tracking-widest text-ora-muted mb-1">Category</p>
        <h1 className="text-2xl font-semibold text-ora-charcoal">{category.name}</h1>
      </div>

      {posts.length === 0 ? (
        <p className="text-sm text-ora-muted">No posts in this category.</p>
      ) : (
