...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const post = await fetchPublicPost("ar", slug);
...
export default async function ArBlogPostPage({ params }: Props) {
  const { slug } = await params;
  const post = await fetchPublicPost("ar", slug);
  ...
  const relatedPosts = await fetchRelatedPosts(post.id, "ar", 3);
  const postUrl = `${baseUrl}/ar/blog/${post.slug}`;
    ...
    <main className="mx-auto max-w-4xl px-4 py-12">
      ...
      <article>
        ...
        {post.tags && post.tags.length > 0 && (
          <div className="mt-10 flex flex-wrap items-center gap-2 border-t border-ora-sand pt-6">
            <span className="text-xs font-medium text-ora-charcoal-light">الوسوم:</span>
            {post.tags.map((tag) => (
              <Link
                key={tag.id}
      ...
      {relatedPosts.length > 0 && (
        <section className="mt-16 border-t border-ora-sand pt-10">
          <h2 className="text-xl font-semibold text-ora-charcoal mb-6">مقالات ذات صلة</h2>
