...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const { tag } = await fetchPostsByTag("ar", slug, 1);

  if (!tag) {
    return { title: "الوسم غير موجود" };
  }

  return generateBlogMetadata({
    metaTitle: `${tag.name} — المدونة`,
    metaDescription: `مقالات بوسم ${tag.name}`,
    slug: `blog/tag/${slug}`,
    locale: "ar",
    postType: "blog",
  });
...
export default async function ArTagArchivePage({ params, searchParams }: Props) {
  const { slug } = await params;
  const { page: pageParam } = await searchParams;
  const page = Math.max(1, parseInt(pageParam || "1", 10) || 1);
  const pageSize = 12;

  const { posts, total, tag } = await fetchPostsByTag("ar", slug, page, pageSize);
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-12">
      <div className="mb-10">
        <p className="text-xs font-medium uppercase tracking-widest text-ora-muted mb-1">الوسم</p>
        <h1 className="text-2xl font-semibold text-ora-charcoal">{tag.name}</h1>
      </div>

      {posts.length === 0 ? (
        <p className="text-sm text-ora-muted">لا توجد مقالات بهذا الوسم.</p>
      ) : (
      ...
      {totalPages > 1 && (
        <nav className="mt-10 flex items-center justify-center gap-2" aria-label="التنقل بين الصفحات">
          {page > 1 && (
            <Link
              href={`/ar/blog/tag/${slug}?page=${page - 1}`}
