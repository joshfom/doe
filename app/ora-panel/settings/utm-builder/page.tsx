...
function QRCodeDialog({
  ...
  const handleDownloadPNG = (resolution: number) => {
    import('@/lib/analytics/qr-code').then(({ generateQRCodeSVG }) => {
      const svgString = generateQRCodeSVG(url, resolution);
      ...
      img.onload = () => {
        ...
        link.download = `qr-${campaign || 'code'}-${resolution}px.png`;
  ...
  const handleDownloadSVG = () => {
    import('@/lib/analytics/qr-code').then(({ generateQRCodeSVG }) => {
      const svgString = generateQRCodeSVG(url, 800);
      const blob = new Blob([svgString], { type: 'image/svg+xml;charset=utf-8' });
      const link = document.createElement('a');
      link.download = `qr-${campaign || 'code'}.svg`;
  ...
  return (
    ...
    >
      <div
        className="relative w-full max-w-md bg-ora-white p-6 shadow-xl border border-ora-sand/60"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Close button */}
        <button
          onClick={onClose}
          className="absolute right-4 top-4 inline-flex h-7 w-7 items-center justify-center text-ora-muted hover:text-ora-charcoal transition-colors"
          aria-label="Close"
        >
          <X className="h-4 w-4" />
        </button>
        ...
        {/* Header */}
        <div className="mb-5">
          <h3 className="text-base font-semibold text-ora-charcoal">QR Code</h3>
          <p className="mt-0.5 text-xs text-ora-charcoal-light">
            Download for print — flyers, rollups, event materials
          </p>
        </div>

        {/* Campaign info */}
        <div className="mb-4 rounded bg-ora-sand/30 px-3 py-2">
          <p className="text-[11px] font-medium text-ora-charcoal-light uppercase tracking-wide">
            Campaign
          </p>
          <p className="text-sm font-medium text-ora-charcoal">{campaign}</p>
        </div>
        ...
        <div className="mb-5 flex items-center gap-2">
          <code className="flex-1 break-all rounded bg-ora-sand/30 px-3 py-2 text-[10px] text-ora-charcoal leading-relaxed">
            {url}
          </code>
          <button
            onClick={handleCopy}
            className="inline-flex h-8 w-8 shrink-0 items-center justify-center border border-ora-stone text-ora-charcoal hover:bg-ora-sand/30 transition-colors"
            title="Copy URL"
          >
            {copied ? (
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-3 gap-2">
            <button
              ...
              className="inline-flex h-9 items-center justify-center gap-1.5 border border-ora-stone text-xs font-medium text-ora-charcoal hover:bg-ora-sand/30 transition-colors"
            >
              <Download className="h-3.5 w-3.5" />
              PNG 300px
            </button>
            <button
              onClick={() => handleDownloadPNG(800)}
              className="inline-flex h-9 items-center justify-center gap-1.5 border border-ora-stone text-xs font-medium text-ora-charcoal hover:bg-ora-sand/30 transition-colors"
            >
              <Download className="h-3.5 w-3.5" />
              PNG 800px
          ...
          <p className="text-[10px] text-ora-muted">
            Use SVG for infinite scale (large banners). PNG 800px works for most flyers and rollups.
          </p>
        </div>
      </div>
    </div>
