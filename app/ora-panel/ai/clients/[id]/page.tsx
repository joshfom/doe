...
export default function EditClientPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: client, isLoading } = useQuery({
    queryKey: ['ai-client', id],
    queryFn: () => fetch(`/api/ai/clients/${id}`).then((r) => {
  ...
  useEffect(() => {
    ...
    setPreferredLanguage(client.preferredLanguage ?? 'en');
    setNotes(client.notes ?? '');
  }, [client]);

  const update = useMutation({
    mutationFn: (body: Record<string, unknown>) =>
      fetch(`/api/ai/clients/${id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(body),
      }).then((r) => { if (!r.ok) throw new Error('Failed'); return r.json(); }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['ai-client', id] });
      queryClient.invalidateQueries({ queryKey: ['ai-clients'] });
      setSaved(true);
  ...
  const remove = useMutation({
    mutationFn: () => fetch(`/api/ai/clients/${id}`, { method: 'DELETE' }).then((r) => { if (!r.ok) throw new Error('Failed'); return r.json(); }),
  ...
  const handleSave = () => {
    ...
    update.mutate({
      ...
      nationality: nationality.trim() || undefined, preferredLanguage,
  ...
  return (
    <div>
      <nav className="mb-6 flex items-center gap-2 text-sm text-ora-muted">
        ...
        <ChevronRight className="h-3.5 w-3.5 stroke-1" />
        <span className="text-ora-charcoal font-medium">Edit Client</span>
      </nav>

      <div className="mb-6 flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-ora-charcoal">Edit Client</h1>
          <p className="mt-1 text-sm text-ora-charcoal-light">Update client record</p>
        </div>
        {confirmDelete ? (
        ...
        <div className="border border-ora-sand/60 bg-ora-white p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="h-10 w-full border border-ora-stone bg-ora-white px-4 text-sm text-ora-charcoal placeholder:text-ora-muted focus-visible:ring-1 focus-visible:ring-ora-gold focus-visible:outline-none" />
            </div>
            <div>
              <label className="mb-1.5 block text-xs font-medium text-ora-charcoal-light">Phone</label>
              <input type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} className="h-10 w-full border border-ora-stone bg-ora-white px-4 text-sm text-ora-charcoal placeholder:text-ora-muted focus-visible:ring-1 focus-visible:ring-ora-gold focus-visible:outline-none" />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="mb-1.5 block text-xs font-medium text-ora-charcoal-light">Nationality</label>
              <input type="text" value={nationality} onChange={(e) => setNationality(e.target.value)} className="h-10 w-full border border-ora-stone bg-ora-white px-4 text-sm text-ora-charcoal placeholder:text-ora-muted focus-visible:ring-1 focus-visible:ring-ora-gold focus-visible:outline-none" />
            </div>
            <div>
