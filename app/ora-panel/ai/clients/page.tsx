...
export default function ClientsPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['ai-clients', page, search],
    queryFn: () => {
      const params = new URLSearchParams({ page: String(page), limit: '20' });
      if (search) params.set('search', search);
      return fetch(`/api/ai/clients?${params}`).then((r) => r.json());
    },
  });
  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-ora-charcoal">AI Clients</h1>
          <p className="mt-1 text-sm text-ora-charcoal-light">Manage client records for AI identification</p>
        </div>
        <Link
          href="/ora-panel/ai/clients/new"
      ...
      {isLoading ? (
        ...
        <div className="border border-ora-sand/60 bg-ora-white p-12 text-center">
          ...
          <p className="mt-2 text-sm text-ora-muted">No clients found</p>
        </div>
      ) : (
        <div className="space-y-2">
          {clients.map((client: any) => (
            <Link
              key={client.id}
              href={`/ora-panel/ai/clients/${client.id}`}
